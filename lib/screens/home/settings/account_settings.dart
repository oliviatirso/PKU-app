  ...
  Future<void> _viewOrDownloadPdf({bool open = false}) async {
    ...
    if (pdfPath == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No profile summary found.'),
    ...
    try {
      ...
      final file = await File(
        '${tempDir.path}/${pdfPath.split('/').last}',
      ...
      if (open) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'File opened. "Save As" in your PDF viewer to download.',
              ),
            ),
          );
          await OpenFile.open(file.path);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error getting PDF: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Future<void> _sharePdf() async {
    ...
    if (pdfPath == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No profile summary found to share.'),
            backgroundColor: Colors.orange,
          ),
        );
      }
    ...
    try {
      ...
      await Share.shareXFiles([
        XFile(file.path),
      ], text: 'Here is my PKU Wise Profile Summary.');
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<void> _logout() async {
    ...
    if (mounted) {
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(
          builder: (_) => const OnboardingScreen(onToggleTheme: null),
        ),
        (_) => false,
      );
    }
      ...
      body: _isLoading
          ...
          : ListView(
              ...
              children: [
                    ...
                    child: Row(
                      ...
                      children: [
                        TextButton.icon(
                          ...
                          onPressed: () => _viewOrDownloadPdf(open: true),
                        ),
                        TextButton.icon(
                          icon: const Icon(Icons.download),
                          label: const Text('Download'),
                          onPressed: () => _viewOrDownloadPdf(open: false),
                        ),
                        TextButton.icon(
                ...
                _buildInfoTile(
                  'Date of Birth',
                  _profileData!['dob']?.split('T')[0] ?? 'N/A',
                ),
                _buildInfoTile('Gender', _profileData!['gender'] ?? 'N/A'),
                _buildInfoTile(
                  'Weight',
                  '${_profileData!['weight_kg']?.toStringAsFixed(1) ?? 'N/A'} kg',
                ),
                _buildInfoTile(
                  'Height',
                  '${_profileData!['height_cm']?.toStringAsFixed(1) ?? 'N/A'} cm',
                ),
                const SizedBox(height: 24),
                _buildSectionHeader('Diagnosis Information'),
                _buildInfoTile(
                  'PKU Severity',
                  _profileData!['pku_severity'] ?? 'N/A',
                ),
                _buildInfoTile(
                  'Diagnosis Date',
                  _profileData!['diagnosis_date']?.split('T')[0] ?? 'N/A',
                ),
                _buildInfoTile(
                  'Primary Hospital / Clinic',
                  _profileData!['metabolic_center'] ?? 'N/A',
                ),
                _buildInfoTile(
                  'Diet Type',
                  _profileData!['diet_type'] ?? 'N/A',
                ),
                _buildInfoTile(
                  'Daily PHE Tolerance',
                  '${_profileData!['phe_tolerance_mg'] ?? 'N/A'} mg',
                ),
                const SizedBox(height: 24),
